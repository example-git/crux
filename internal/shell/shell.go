...
// CruxEnvMarkers returns a fresh slice of the environment variables that
// Crux unconditionally sets on every shell it spawns — both the interactive
...
func NewShell(opts *Options) *Shell {
	...
	// binaries and nested Crux instances) can't attach to or release
	// the parent pane's agent authority.
	env = withoutHerdrEnv(env)

	// Allow tools to detect execution by Crux.
	env = append(env, CruxEnvMarkers()...)
