...
type RunOptions struct {
	...
	// the Crux process cwd — hooks and the bash tool have different
...
// Crux handler stack. Shared by the stateless [Run] entrypoint and the
...
// regardless of the caller's environment because Crux shells are never
...
//  1. builtins first (so Crux's in-process jq wins over any PATH binary);
...
// builtinHandler returns middleware that dispatches recognized Crux
