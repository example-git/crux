...
// one. When local, `crux run --channels server:webhook` fails with
...
func TestChannelsFlagAvailableOnRunCmd(t *testing.T) {
	...
	require.NotNil(t, flag, "the --channels flag must be available on `crux run` (register it as a persistent flag on rootCmd)")
...
func TestChannelsFlagAvailableOnRootCmd(t *testing.T) {
	...
	require.NotNil(t, flag, "the --channels flag must be available on `crux` (rootCmd)")
}
