...
// with reason=tool_use and used to terminate `crux run` early via
...
// subscribers (e.g. a TUI watching workspace events while `crux
...
// non-nil error from `crux run` so shells and CI catch it via
// exit status.
...
// run (e.g. Ctrl+C while `crux run` waits) exits cleanly rather
...
// queue scenario: `crux run --continue` attaches to a session
