...
func TestRemoteCollectionIgnoresLaunchDirectoryModelConfig(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(config.GlobalConfigData(), global, 0o600))
	root := t.TempDir()
	t.Chdir(root)
	...
	require.NoError(t, os.MkdirAll(filepath.Dir(workspacePath), 0o700))
	require.NoError(t, os.WriteFile(workspacePath, project, 0o600))
