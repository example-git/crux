...
func TestProviderInstructionsThroughTLS(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(serverConfig, serverConfigBytes, 0600))
	...
	require.NoError(t, os.WriteFile(clientConfig, []byte(`{"providers":{"example-responses":{"api_key":"synthetic-instructions-access","plugin":{"id":"example.responses-oauth","version":"1.2.0"},"configuration":{"oauth_client_id":"synthetic-client"}}},"models":{"large":{"provider":"example-responses","model":"example-reasoner"},"small":{"provider":"example-responses","model":"example-small"}}}`), 0600))
	instructionDir := filepath.Join(clientHome, ".ai-cli", "instructions")
	require.NoError(t, os.MkdirAll(instructionDir, 0700))
	...
	require.NoError(t, os.WriteFile(instructionFile, []byte(initialContext), 0600))
	...
	require.NoError(t, os.WriteFile(instructionFile, []byte(editedContext), 0600))
...
func installInstructionsFixture(t *testing.T, endpoint, dataDir, cacheDir, text string) {
	...
	require.NoError(t, os.WriteFile(filepath.Join(source, "instructions", "native.txt"), []byte(text), 0600))
