...
func TestWorkspaceNativeCheckedAPIKeyTLSAndInference(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(filepath.Join(values["CRUX_GLOBAL_CONFIG"], "crux.json"), []byte(cfg), 0600))
	...
	result, err := receiver.App.CurrentAgentCoordinator().Model().Model.Generate(t.Context(), fantasy.Call{Headers: map[string]string{"x-session-id": "native-checked-session"}, Prompt: fantasy.Prompt{fantasy.NewUserMessage("native checked literal")}})
	require.NoError(t, err)
	require.NotNil(t, result)
	require.Equal(t, "native key accepted", result.Content[0].(fantasy.TextContent).Text)
	require.EqualValues(t, 1, checks.Load())
	stream, err := receiver.App.CurrentAgentCoordinator().Model().Model.Stream(t.Context(), fantasy.Call{Headers: map[string]string{"x-session-id": "native-checked-stream"}, Prompt: fantasy.Prompt{fantasy.NewUserMessage("native checked stream")}})
	require.NoError(t, err)
