...
func TestRemoteCodebaseIndexBuildSearchAndSettingsThroughTLS(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(filepath.Join(clientAuth, "codebase-index-auth.json"), []byte(`{"accessToken":"synthetic-client-index-token","authMode":"vscode"}`), 0600))
	require.NoError(t, os.WriteFile(filepath.Join(serverAuth, "codebase-index-auth.json"), []byte(`{"accessToken":"synthetic-server-must-not-be-used","authMode":"vscode"}`), 0600))
...
func TestLocalCodebaseIndexPreservesConfiguredPaths(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(config.GlobalConfigData(), []byte(`{"providers":{"fixture":{"type":"openai-compat","api_key":"synthetic-model-token","base_url":"https://fixture.invalid/v1","models":[{"id":"fixture","name":"Fixture"}]}},"models":{"large":{"provider":"fixture","model":"fixture"},"small":{"provider":"fixture","model":"fixture"}}}`), 0600))
