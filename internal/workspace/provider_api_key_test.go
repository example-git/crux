	...
	for _, mode := range []string{"acknowledged", "lost-response", "rejected", "shadowed"} {
		t.Run(mode, func(t *testing.T) {
			...
			if mode == "rejected" {
				...
				_, recoveryErr = receiver.App.CurrentAgentCoordinator().Model().Model.Generate(t.Context(), fantasy.Call{Prompt: fantasy.Prompt{fantasy.NewUserMessage("recovered checked key")}})
				require.NoError(t, recoveryErr)
				observed := f.observed()
