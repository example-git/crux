	...
	for _, mode := range []string{"acknowledged", "lost-response", "rejected", "shadowed"} {
		t.Run(mode, func(t *testing.T) {
			...
			if mode != "shadowed" {
				...
				require.NoError(t, readErr)
				require.NoError(t, os.WriteFile(projectPath, projectData, 0o600))
			...
			if mode == "rejected" {
				...
				recovery := ProviderAuthenticationRecoveryRequest{OperationID: save.OperationID, Target: save.Target, RecoveryID: strings.Repeat("c", 32), RecoverySequence: 1}
				recovered, recoveryErr := f.w.RecoverProviderAuthentication(t.Context(), recovery)
				require.NoError(t, recoveryErr)
				require.NoError(t, recovered.ValidateAPIKeySave(save))
				require.Equal(t, outcome, recovered)
				require.Equal(t, baseline+2, f.puts.Load(), "explicit recovery may publish one retained proposal")
				recoveredAgain, recoveryErr := f.w.RecoverProviderAuthentication(t.Context(), recovery)
				require.NoError(t, recoveryErr)
				require.Equal(t, recovered, recoveredAgain)
				require.Equal(t, baseline+2, f.puts.Load())
				_, recoveryErr = receiver.CurrentAgentCoordinator().Model().Model.Generate(t.Context(), fantasy.Call{Prompt: fantasy.Prompt{fantasy.NewUserMessage("recovered checked key")}})
				require.NoError(t, recoveryErr)
				observed := f.observed()
				...
				_, err = receiver.CurrentAgentCoordinator().Model().Model.Generate(t.Context(), fantasy.Call{Prompt: fantasy.Prompt{fantasy.NewUserMessage("checked key receiver")}})
				require.NoError(t, err)
				observed := f.observed()
				require.Equal(t, "Bearer "+literal, observed[len(observed)-1])
			}
