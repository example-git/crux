...
func TestProviderAuthRejectsUnadmittedWorkBeforeServiceCreation(t *testing.T) {
	for _, mode := range []string{"request canceled", "workspace canceled", "workspace closing", "workspace missing"} {
		t.Run(mode, func(t *testing.T) {
			...
			checked, err := b.CheckProviderAPIKey(ctx, id, providerauth.APIKeyCheckRequest{CheckID: strings.Repeat("d", 32), Target: target, CredentialID: "provider.api_key", Source: "synthetic-secret"})
			require.ErrorIs(t, err, expected)
			require.Equal(t, target, checked.Outcome.Previous)
			require.Nil(t, checked.Outcome.CheckedTarget)
			saved, err := b.SaveCheckedProviderAPIKey(ctx, id, providerauth.APIKeySaveRequest{OperationID: strings.Repeat("e", 32), CheckID: strings.Repeat("d", 32), Target: target})
			require.ErrorIs(t, err, expected)
			require.Equal(t, strings.Repeat("d", 32), saved.Outcome.CheckID)
			require.Zero(t, saved.Outcome.Progress)
			require.Nil(t, saved.Workspace)
