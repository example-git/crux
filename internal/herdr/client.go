...
// multiplexer. When Crux runs inside a herdr-managed pane it reports
...
// Client reports Crux agent state to a running herdr instance.
type Client struct {
	socketPath string
	paneID     string
...
// call from environment variables. Returns nil when Crux is not
...
// is not strictly greater. Because crux seeds seq from the wall
// clock at startup (see newFromEnv), a restarted crux in the same
...
func (c *Client) newRequestLocked(method, idPrefix, state string) reportRequest {
	...
	return reportRequest{
		ID:     fmt.Sprintf("crux:%s:%d", idPrefix, time.Now().UnixNano()),
		Method: method,
		Params: reportParams{
			PaneID:         c.paneID,
			Source:         "crux",
