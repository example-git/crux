...
const CruxLogsToolName = "crux_logs"

//go:embed crux_logs.md.tpl
var cruxLogsDescriptionTmpl []byte

var cruxLogsDescriptionTpl = template.Must(
	template.New("cruxLogsDescription").
		Parse(string(cruxLogsDescriptionTmpl)),
)

type cruxLogsDescriptionData struct {
	DefaultLines int
	MaxLines     int
}

func cruxLogsDescription() string {
	return renderTemplate(cruxLogsDescriptionTpl, cruxLogsDescriptionData{
...
type CruxLogsParams struct {
	Lines int `json:"lines,omitempty" description:"Number of recent log entries to return (default 50, max 100)"`
}

func NewCruxLogsTool(logFile string) fantasy.AgentTool {
	return fantasy.NewAgentTool(
		CruxLogsToolName,
		cruxLogsDescription(),
		func(ctx context.Context, params CruxLogsParams, call fantasy.ToolCall) (fantasy.ToolResponse, error) {
			result := runCruxLogs(logFile, params)
			return fantasy.NewTextResponse(result), nil
		},
	)
}

// runCruxLogs reads and formats the last N log entries from the given file.
func runCruxLogs(logFile string, params CruxLogsParams) string {
