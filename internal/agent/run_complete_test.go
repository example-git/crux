...
// event into a closure nobody reads, and subscribers (`crux run`)
...
func TestDrainQueueForStep_FiltersUnderDispatchLock(t *testing.T) {
	...
	fold, canceledDrops := a.drainQueueForStep(sessionID)
...
func TestDrainQueueForStep_ReportsCanceledRunIDDrops(t *testing.T) {
	...
	require.Len(t, canceledDrops, 2)
	require.Equal(t, "run-canceled", canceledDrops[0].RunID)
	require.Empty(t, canceledDrops[1].RunID)
...
func TestCanceledQueueDropInvokesDiscardCallbackWithoutRunID(t *testing.T) {
	env := testEnv(t)
	agent := NewSessionAgent(SessionAgentOptions{Sessions: env.sessions, Messages: env.messages}).(*sessionAgent)
	called := 0
	agent.publishCanceledQueueDrops([]SessionAgentCall{{OnDiscarded: func() { called++ }}})
...
// than internal bookkeeping, which is the contract a `crux run` caller
...
// bookkeeping slice, not the published event a `crux run` caller awaits.
...
func TestDrainQueueForStep_DroppedRunIDPublishesCancelledRunComplete(t *testing.T) {
	...
	_, canceledDrops := a.drainQueueForStep(sessionID)
	require.Len(t, canceledDrops, 2)
