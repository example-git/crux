...
func newDetachedHookedTool(inner fantasy.AgentTool, runner *hooks.Runner) *hookedTool {
	return &hookedTool{inner: inner, runner: runner, detachedOnly: true}
}

// wrapToolsWithHooks returns a tool slice with each entry wrapped in a
// hookedTool. Sub-agent wrappers activate only for detached execution.
func wrapToolsWithHooks(tools []fantasy.AgentTool, runner *hooks.Runner, isSubAgent bool) []fantasy.AgentTool {
	...
	for i, tool := range tools {
		if isSubAgent {
			out[i] = newDetachedHookedTool(tool, runner)
		} else {
...
func (h *hookedTool) Run(ctx context.Context, call fantasy.ToolCall) (fantasy.ToolResponse, error) {
	if h.detachedOnly && !permission.IsDetachedAgent(ctx) {
