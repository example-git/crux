...
func TestCruxIgnore(t *testing.T) {
	tempDir := t.TempDir()
	t.Chdir(tempDir)
	...
	// Create a .cruxignore file that ignores .log files
	require.NoError(t, os.WriteFile(".cruxignore", []byte("*.log\n"), 0o644))
...
func TestShouldExcludeFile(t *testing.T) {
	...
	// Create .cruxignore file
	cruxignoreContent := "custom_ignored/\n"
	if err := os.WriteFile(filepath.Join(tempDir, ".cruxignore"), []byte(cruxignoreContent), 0o644); err != nil {
		t.Fatalf("Failed to create .cruxignore: %v", err)
	}
	...
	require.True(t, ShouldExcludeFile(tempDir, customIgnored), "Expected custom_ignored to be ignored by .cruxignore")
...
func TestShouldExcludeFileHierarchical(t *testing.T) {
	...
	// Create .cruxignore in subdir that ignores normal_nested
	subCruxignore := "normal_nested/\n"
	if err := os.WriteFile(filepath.Join(subDir, ".cruxignore"), []byte(subCruxignore), 0o644); err != nil {
		t.Fatalf("Failed to create subdir .cruxignore: %v", err)
	}

	// Test hierarchical ignore behavior - this should work because the .cruxignore is in the parent directory
	require.True(t, ShouldExcludeFile(tempDir, nestedNormal), "Expected normal_nested to be ignored by subdir .cruxignore")
