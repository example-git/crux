...
func (w *checkedKeyUIWorkspace) result(r providerauth.APIKeySaveRequest) (providerauth.MutationOutcome, error) {
	...
	target := r.Target
	target.Generation.Sequence++
	status := providerauth.Status{Owner: target.Owner, Configured: true, AccountState: "none", Credentials: []providerauth.CredentialStatus{{Kind: "api-key", State: "configured"}, {Kind: "oauth", State: "absent"}}}
...
func newCheckedKeyUI(t *testing.T) (*UI, *checkedKeyUIWorkspace, dialog.ActionSelectModel) {
	...
	status := providerauth.Status{Owner: owner, Configured: true, AccountState: "none", Credentials: []providerauth.CredentialStatus{{Kind: "api-key", State: "configured"}, {Kind: "oauth", State: "absent"}}}
...
func checkKeyInput(t *testing.T, ui *UI, ws *checkedKeyUIWorkspace, d *dialog.APIKeyInput, source string) apiKeyCheckMsg {
	...
	require.IsType(t, dialog.ActionAPIKeyCheck{}, action)
	ids := runCheckedKeyCmd(ws, ui.handleDialogAction(action))
	require.Len(t, ids, 1)
...
func saveKeyInput(t *testing.T, ui *UI, ws *checkedKeyUIWorkspace, d *dialog.APIKeyInput) apiKeySaveMsg {
	t.Helper()
	action := d.HandleMsg(tea.KeyPressMsg{Code: tea.KeyEnter})
	require.IsType(t, dialog.ActionAPIKeySave{}, action)
	messages := runCheckedKeyCmd(ws, ui.handleDialogAction(action))
	require.Len(t, messages, 1)
...
func TestCheckedKeyUICancelAndWrongCheckCannotSave(t *testing.T) {
	for _, mode := range []string{"close-before-ID", "close-before-check", "wrong-check", "check-error"} {
		t.Run(mode, func(t *testing.T) {
			ui, ws, selection := newCheckedKeyUI(t)
			d := openCheckedKey(t, ui, ws, selection)
			d.HandleMsg(tea.PasteMsg{Content: "source-A"})
			ids := runCheckedKeyCmd(ws, ui.handleDialogAction(d.HandleMsg(tea.KeyPressMsg{Code: tea.KeyEnter})))
