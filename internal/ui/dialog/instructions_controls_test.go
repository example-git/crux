...
func TestInstructionsRuntimeControlsResolveWithoutOptimisticModelMutation(t *testing.T) {
	...
	row := runtimeControlRowValue(d.items[d.cursor])
	require.Contains(t, row, "true · provider · inherited")
	require.NotContains(t, row, "Unavailable")
	require.NotContains(t, row, "runtime control configuration path")
