...
// already in use by another crux process.
var ErrDataDirLocked = errors.New("data directory already in use by another crux process")

// dataDirLockFile is the name of the lock file inside the data
// directory. It lives next to crux.db so users can `ls` and find it.
const dataDirLockFile = "crux.lock"
...
// {dataDir}/crux.lock. If the lock is already held by another
...
// CRUX_SKIP_DATADIR_LOCK is set to a truthy value. This is intended
...
func skipDataDirLock() bool {
	v, _ := strconv.ParseBool(os.Getenv("CRUX_SKIP_DATADIR_LOCK"))
	return v
}
