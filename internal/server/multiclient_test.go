...
func TestWorkspaceChannelRejectsMissingClientID(t *testing.T) {
	t.Parallel()
	c := newTestController()

	req := httptest.NewRequestWithContext(t.Context(), http.MethodGet, "/v1/workspaces/abc/channel", nil)
	req.SetPathValue("id", "abc")
	req.Header.Set("Crux-Runtime-Protocol", proto.RemoteRuntimeProtocol)
	req.Header.Set(cruxlog.EphemeralStateHeader, "1")
	rec := httptest.NewRecorder()

	c.handleGetWorkspaceChannel(rec, req)

	require.Equal(t, http.StatusBadRequest, rec.Code)
}

func TestWorkspaceChannelRejectsMalformedClientID(t *testing.T) {
	t.Parallel()
	c := newTestController()

	req := httptest.NewRequestWithContext(t.Context(), http.MethodGet, "/v1/workspaces/abc/channel?client_id=nope", nil)
	req.SetPathValue("id", "abc")
	req.Header.Set("Crux-Runtime-Protocol", proto.RemoteRuntimeProtocol)
	req.Header.Set(cruxlog.EphemeralStateHeader, "1")
	rec := httptest.NewRecorder()
