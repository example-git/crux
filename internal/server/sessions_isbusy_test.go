...
func (s *stubCoordinator) QueuedPromptsList(string) []agent.QueuedPrompt { return nil }
...
func (s *stubSessions) SetMode(ctx context.Context, id string, mode session.Mode) error {
	return s.SetPlanState(ctx, id, mode, "")
}

func (s *stubSessions) SetPlanState(_ context.Context, id string, mode session.Mode, plan string) error {
	for i := range s.all {
		if s.all[i].ID == id {
			s.all[i].Mode = mode
			s.all[i].Plan = plan
...
func TestPutWorkspaceSessionMode(t *testing.T) {
	const sessionID = "s-mode"
	controller, workspaceID := buildBusyWorkspace(t, sessionID, false)
	body := bytes.NewBufferString(`{"mode":"plan"}`)
	req := httptest.NewRequestWithContext(t.Context(), http.MethodPut, "/v1/workspaces/"+workspaceID+"/sessions/"+sessionID+"/mode", body)
	req.SetPathValue("id", workspaceID)
	req.SetPathValue("sid", sessionID)
	rec := httptest.NewRecorder()

	controller.handlePutWorkspaceSessionMode(rec, req)
	require.Equal(t, http.StatusOK, rec.Code)

	var got proto.Session
	require.NoError(t, json.Unmarshal(rec.Body.Bytes(), &got))
	require.Equal(t, sessionID, got.ID)
	require.Equal(t, string(session.ModePlan), got.Mode)
}

func TestPutWorkspaceSessionModeRejectsInvalidMode(t *testing.T) {
	const sessionID = "s-mode"
	controller, workspaceID := buildBusyWorkspace(t, sessionID, false)
	...
	var got proto.Error
	require.NoError(t, json.Unmarshal(rec.Body.Bytes(), &got))
	require.Equal(t, `invalid session mode "invalid"`, got.Message)
}
