...
func TestTracker_BuiltinSkillTracking(t *testing.T) {
	...
	// Simulate active skills including a builtin skill (crux-config).
	activeSkills := []*Skill{
		{Name: "crux-config", Description: "Crux config", Builtin: true},
		{Name: "go-doc", Description: "Go docs", Builtin: false},
	}
	tracker := NewTracker(activeSkills)

	// Initially not loaded.
	require.False(t, tracker.IsLoaded("crux-config"))
	require.False(t, tracker.IsLoaded("go-doc"))

	// Mark builtin skill as loaded (simulating read via crux://...).
	tracker.MarkLoaded("crux-config")
	require.True(t, tracker.IsLoaded("crux-config"))
