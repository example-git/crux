...
These prompt recipes apply to every request handled through the native `imagegen` tool.
...
The labeled lines are prompt scaffolding, not a closed schema. `Asset type` is prompt-only context; actual image paths belong in edit mode's `images` parameter.

Native tool parameters such as `quality`, `images`, `size`, `background`, and local output paths are documented in `references/cli.md` and `references/image-api.md`.

Model notes:
- `gpt-image-2` is the default when using the configured Codex account.
- `gpt-image-2` supports `quality` values `low`, `medium`, `high`, and `auto`.
- For 4K-style `gpt-image-2` output, use `3840x2160` or `2160x3840`.
- Try `background: transparent` for transparent output; if the endpoint does not honor it, fall back to the chroma-key workflow in `references/image-api.md`.
- Do not set an input-fidelity override; image inputs already use high fidelity.
