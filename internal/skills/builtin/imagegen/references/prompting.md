...
These prompting principles apply to every request handled through the native `imagegen` tool.

This file is about prompt structure, specificity, and iteration. Execution controls such as `quality`, image inputs, output size, and output paths live in `references/cli.md` and `references/image-api.md`.
...
- If the user provides images for style, composition, or mood guidance, use native edit mode and pass them in the ordered `images` list, even when the desired output is a new scene rather than a modification of the reference.
...
## Transparent images
- Try `background: transparent` for a genuinely transparent background and preserve its alpha; see `references/cli.md`. Fall back to the chroma-key workflow if the endpoint does not honor it.

## Execution controls
- `quality`, image inputs, output size, background behavior, and output paths are documented in `references/cli.md` and `references/image-api.md`.
...
- `gpt-image-2` always uses high fidelity for image inputs, so there is no fidelity flag to set.
