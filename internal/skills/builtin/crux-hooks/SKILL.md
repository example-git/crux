---
name: crux-hooks
description: Use when the user wants to add, write, debug, or configure a Crux hook — gating or blocking tool calls, approving or rewriting tool input before execution, injecting context into tool results, or troubleshooting hook behavior in crux.json.
---

# Crux Hooks

Hooks are user-defined commands in `crux.json` that fire at specific points
during execution, giving deterministic control over tool behavior. They run
**before** permission checks for top-level tool calls and detached background-agent
tool calls. Synchronous delegated agents are represented by their outer tool call
and do not recursively run the parent's hooks for every child call.
...
| `CRUX_TOOL_NAME`            | Tool being called (e.g. `bash`)          |
| `CRUX_SESSION_ID`           | Current session ID                       |
| `CRUX_CWD`                  | Working directory                        |
| `CRUX_PROJECT_DIR`          | Project root directory                   |
| `CRUX_TOOL_INPUT_COMMAND`   | For `bash` calls: the shell command      |
| `CRUX_TOOL_INPUT_FILE_PATH` | For file tools: the target file path     |
...
if echo "$CRUX_TOOL_INPUT_COMMAND" | grep -qE 'rm\s+-(rf|fr)\s+/'; then
...
3. Add the entry under `hooks.PreToolUse` in `crux.json` with the right matcher.
...
- Non-zero exit codes other than 2/49 are logged but don't block — check Crux logs.
...
Crux also accepts Claude Code's `hookSpecificOutput` envelope. One intentional
divergence: Crux treats `updated_input` as shallow-merge, Claude Code replaces.
