---
name: crux-config
description: Use when the user needs help configuring Crux — writing cruxrc (the Bash config format) or crux.json, setting up providers, models, LSPs, MCP servers, hooks, skills, permissions, or changing Crux behavior.
---

# Crux Configuration

Crux supports two config formats:

- **`cruxrc`** — a Bash script that builds config by calling Crux builtins.
  **Preferred.** Because it is real Bash you get includes, secrets,
  conditionals, and variables for free.
- **`crux.json`** — static JSON. Fully supported; see
  [Legacy JSON format](#legacy-json-format).

Both are discovered together and deep-merged. Priority (highest to lowest):

1. `.cruxrc` / `cruxrc` / `.crux.json` / `crux.json` (project-local,
   closer-to-cwd wins; Windows uses `.\.cruxrc` / `.\cruxrc`)
2. `$XDG_CONFIG_HOME/crux/cruxrc` or `~/.ai-cli/crux/cruxrc`
   (`%XDG_CONFIG_HOME%\crux\cruxrc` or
   `%USERPROFILE%\.config\crux\cruxrc` on Windows)

Data directories (`~/.ai-cli/data/crux` and `%LOCALAPPDATA%\crux`) contain
machine-owned JSON state only; Crux does not discover or execute a `cruxrc`
from those locations.

If a directory has both `cruxrc` and `crux.json`, they merge (`cruxrc` wins
on conflicts) and Crux logs a warning.

## cruxrc at a glance

A `cruxrc` is a plain Bash script executed at load time with the same embedded
...
`CRUX_VERSION` is exported into the script so you can feature-detect the
running Crux (it is the literal `devel` for local builds):

```bash
[[ "$CRUX_VERSION" != devel ]] && lsp add gopls --command gopls
```
...
`--system-prompt-prefix TEXT`, `--tooling-instructions crux|native`,
...
```bash
provider add <provider-id> --tooling-instructions native
```

This changes system/tooling instruction text. It does not make requests
identical to official clients or guarantee provider-side classification.
...
- `<provider>/<id>` is the same form `crux models` prints. A missing slash is
...
The **MCP Servers** entry in the `ctrl+p` Commands menu provides a
project-scoped form for the same fields. It writes structured
`.crux/crux.json` configuration, reconnects the affected servers, and reports
whether each server connected, needs authentication, or failed validation. It
does not rewrite executable `cruxrc` scripts.
...
```bash
hook add PreToolUse --matcher "^bash$" --command ".crux/hooks/no-haskell.sh" --name no-haskell
```
...
| ---------------------------- | ------------------------------------------------- |
| `CRUX_EVENT`                | Event name (e.g. `PreToolUse`)                    |
| `CRUX_TOOL_NAME`            | Name of the tool being called                     |
| `CRUX_SESSION_ID`           | Current session ID                                |
| `CRUX_CWD`                  | Current working directory                         |
| `CRUX_PROJECT_DIR`          | Project root directory                            |
| `CRUX_TOOL_INPUT_COMMAND`   | Value of `command` from tool input (if present)   |
| `CRUX_TOOL_INPUT_FILE_PATH` | Value of `file_path` from tool input (if present) |
...
Crux also accepts the Claude Code hook output format, so existing hooks work
unchanged:
...
- `CRUX_VERSION` — exported into `cruxrc` at load; the running version (or
  `devel` for local builds).
- `CRUX_GLOBAL_CONFIG` — override global config location.
- `CRUX_GLOBAL_DATA` — override data directory location.
- `CRUX_SKILLS_DIR` — override default skills directory.
- `CRUX_DISABLE_AUTO_MEMORY` — set to a true boolean to disable project-scoped persistent memory, relevance recall, extraction, and consolidation.
- `CRUX_AUTO_MEMORY_DIR` — absolute memory-directory override; custom paths retain normal file permission prompts and disable automatic background writes.

## Legacy JSON format

`crux.json` is the original static format. It still works and merges with
`cruxrc`. Basic structure:
...
### cruxrc ↔ crux.json mapping

| cruxrc                             | crux.json                                             |
| ------------------------------------ | ------------------------------------------------------ |
| `provider add openai --api-key "$K"` | `providers.openai = {"api_key": "$K"}`                 |
| `provider add codex --tooling-instructions native` | `providers.codex.tooling_instructions = "native"` |
...
time (in `cruxrc`, everything is native Bash so this table does not apply):
...
Both formats are trusted code. `cruxrc` runs entirely, and any `$(...)` in
`crux.json` runs at load time, with the invoking user's shell privileges,
before the UI appears. Don't launch Crux in a directory whose config you
haven't reviewed.
