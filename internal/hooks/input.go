...
func BuildEnv(eventName, toolName, sessionID, cwd, projectDir, toolInputJSON string) []string {
	...
	env = append(
		env,
		fmt.Sprintf("CRUX_EVENT=%s", eventName),
		fmt.Sprintf("CRUX_TOOL_NAME=%s", toolName),
		fmt.Sprintf("CRUX_SESSION_ID=%s", sessionID),
		fmt.Sprintf("CRUX_CWD=%s", cwd),
		fmt.Sprintf("CRUX_PROJECT_DIR=%s", projectDir),
	)

	// Extract tool-specific env vars from the JSON input.
	if toolInputJSON != "" {
		if cmd := gjson.Get(toolInputJSON, "command"); cmd.Exists() {
			env = append(env, fmt.Sprintf("CRUX_TOOL_INPUT_COMMAND=%s", cmd.String()))
		}
		if fp := gjson.Get(toolInputJSON, "file_path"); fp.Exists() {
			env = append(env, fmt.Sprintf("CRUX_TOOL_INPUT_FILE_PATH=%s", fp.String()))
		}
	}
...
// Supports both Crux format and Claude Code format (hookSpecificOutput).
