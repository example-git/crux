...
// loadCruxSh writes a crux.sh into an isolated project and loads it through
...
func loadCruxSh(t *testing.T, script string) *config.ConfigStore {
	t.Helper()
	store, err := loadCruxShErr(t, script)
	require.NoError(t, err)
	return store
}

// loadCruxShErr is loadCruxSh without asserting success, for cases that are
// expected to fail at load time.
func loadCruxShErr(t *testing.T, script string) (*config.ConfigStore, error) {
	t.Helper()
	...
	workDir := t.TempDir()
	dataDir := t.TempDir()
	require.NoError(t, os.WriteFile(filepath.Join(workDir, "cruxrc"), []byte(script), 0o644))

	return config.Load(workDir, dataDir, false)
}

func TestShellConfigPermissionsAllow(t *testing.T) {
	store := loadCruxSh(t, `permissions allow bash view`)
...
func TestShellConfigPermissionsAccumulateAndDedup(t *testing.T) {
	store := loadCruxSh(t, `permissions allow bash
...
func TestShellConfigPermissionsDeny(t *testing.T) {
	store := loadCruxSh(t, `permissions deny bash sourcegraph
