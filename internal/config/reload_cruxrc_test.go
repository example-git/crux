...
// isolateReloadEnv points HOME/XDG/CRUX_* at a throwaway directory so a
...
func isolateReloadEnv(t *testing.T) (workDir, dataDir string) {
	...
	t.Setenv("CRUX_GLOBAL_CONFIG", filepath.Join(isolated, ".config", "crux"))
	t.Setenv("CRUX_GLOBAL_DATA", filepath.Join(isolated, ".local", "share", "crux"))
	return t.TempDir(), t.TempDir()
}

// TestReloadFromDisk_PicksUpEditedCruxrc verifies that a cruxrc edit on
// disk is reflected in memory after a reload, matching how JSON config
// already behaves.
func TestReloadFromDisk_PicksUpEditedCruxrc(t *testing.T) {
	workDir, dataDir := isolateReloadEnv(t)
	rcPath := filepath.Join(workDir, "cruxrc")
	...
	require.Equal(t, "osc", store.Config().Options.Notifications,
		"reload must re-execute the cruxrc and pick up the new value")
}

// TestReloadFromDisk_FailingCruxrcKeepsOldConfig verifies that a cruxrc
...
func TestReloadFromDisk_FailingCruxrcKeepsOldConfig(t *testing.T) {
	workDir, dataDir := isolateReloadEnv(t)
	rcPath := filepath.Join(workDir, "cruxrc")
	require.NoError(t, os.WriteFile(rcPath, []byte("option notifications bell\n"), 0o644))
	...
	// Overwrite with a cruxrc that errors at load time (unknown option key).
	require.NoError(t, os.WriteFile(rcPath, []byte("option totally-bogus-key value\n"), 0o644))

	err = store.ReloadFromDisk(context.Background())
	require.Error(t, err, "a failing cruxrc must surface a reload error")
	require.Equal(t, "bell", store.Config().Options.Notifications,
		"in-memory config must be preserved when a reload fails")
}

// TestReloadFromDisk_HangingCruxrcIsInterruptible is the core regression test
// for the reload-path blocker: config reloads run while holding the store's
// write lock, so a cruxrc that blocks (busy loop, hung command substitution)
...
func TestReloadFromDisk_HangingCruxrcIsInterruptible(t *testing.T) {
	workDir, dataDir := isolateReloadEnv(t)
	rcPath := filepath.Join(workDir, "cruxrc")
	require.NoError(t, os.WriteFile(rcPath, []byte("option notifications bell\n"), 0o644))
	...
	// Overwrite with a cruxrc that never terminates.
...
// TestLoad_TracksNotYetCreatedGlobalCruxrc verifies that config.Load tracks
// the global cruxrc path even when the file does not exist yet, so a cruxrc
// created after startup is detected as a staleness change. Previously only
// successfully-loaded paths were tracked, so a mid-session global cruxrc went
// unnoticed until something else triggered a reload.
//
// Project-level cruxrc files are discovered by walking the tree for existing
// files, so a not-yet-created project cruxrc is still only picked up on the
// next reload; the global path is the common case this covers.
func TestLoad_TracksNotYetCreatedGlobalCruxrc(t *testing.T) {
	workDir, dataDir := isolateReloadEnv(t)
	globalRC := filepath.Join(t.TempDir(), "cruxrc")
	t.Setenv("CRUX_GLOBAL_CONFIG", filepath.Dir(globalRC))

	// Load with no global cruxrc present. Staleness tracking must be active even
	// before a provider is configured so first-run configuration changes are seen.
	store, err := config.Load(workDir, dataDir, false)
	require.NoError(t, err)
	require.False(t, store.ConfigStaleness().Dirty, "fresh load should be clean")

	// Create the global cruxrc after startup.
	require.NoError(t, os.WriteFile(globalRC, []byte("option debug true\n"), 0o644))

	staleness := store.ConfigStaleness()
	require.True(t, staleness.Dirty, "creating a global cruxrc must be detected")
