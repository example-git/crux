...
### Checked API-key catalog probes

The `operation.model-catalog-http` host feature enables checked API-key input for a native provider that declares exactly one `model-catalog` operation using `generic-json` over `http-json`. The operation must name its method and literal path. The host uses that operation's endpoint policy, headers, optional client identity, configuration bindings, request and response JSON transforms, and bounded idempotent retries. Credential audiences also restrict access through mapped configuration properties. Inference-only policies, authentication refresh, and streaming idle timeouts are unsupported on a catalog operation and fail activation.

The `manifest-http-200` check policy requires HTTP 200, JSON decoding, and successful response transformation. Requests and responses have a 1 MiB limit; a five-second check budget includes identity resolution, retries, and the response body, and shorter declared request/connect timeouts still apply. A successful check records this evidence; it does not establish permission to run a selected model. Save retains the exact checked input, definition, owner, and literal credential without repeating the probe. Missing or multiple catalog declarations remain visibly unsupported; the host never invents a native `/models` endpoint or substitutes an inference call. Remote runtime compiler v18 negotiates this additional executable manifest behavior.

Every declared `config_property` credential is required to be a nonempty string when the provider is bound for execution, even when its configuration schema omits that field from `required`. A declaration is an executable input contract; do not declare unused optional credentials.

Declared credentials with `config_property` have distinct input slots named `configuration.<credential-id>`. The owner reports these slots alongside `provider.api_key`, with presence metadata only. Check resolves the selected source once against the captured owner environment, validates its configuration schema, and retains the literal privately. Save writes only that literal JSON property's authored source in the selected scope; other property credentials, the primary key, OAuth token, account selection, and models remain unchanged. Generic edits retain untouched resolved sources, while explicit disk reload resolves the saved sources afresh. Public configuration projections remove these credential properties regardless of whether the manifest also marks them secret.

A property check uses the declared catalog probe when that operation admits the selected credential's audience. If no catalog is declared, or the selected field has no catalog audience, the result explicitly reports **source and schema validation only** and no connection probe. Multiple or malformed catalog declarations still fail visibly. This supports fields such as OAuth client credentials without inventing a login or inference probe. The primary API-key check retains its existing connection policy.

When other declared credential properties are absent, setup may save one field at a time. Only absent required credential properties may remain as schema failures; unrelated schema violations are rejected. Such a check reports pending setup and no probe, and Save does not continue a selected model. Explicit provider and model selections remain present but unavailable until setup is complete. The accepted runtime carries the same finite unavailable state, so receiver credentials cannot fill missing fields. Compiler v22 negotiates the property-slot and pending-setup contract; these changes await the project's deferred functionality and transport acceptance phase.
