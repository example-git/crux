...
### Codex Desktop and SkySight

Desktop replacement requires more than the CLI subset above. See the [SkySight helper contract](codex-desktop-skysight.md) for the separately selected nested app-server and `exec` processes, `memories.consolidation_model` precedence and GPT-5.5 fallback, observed model-access failure, current Crux gaps, and required end-to-end checks. The helper's configuration and instruction-isolation flags are not established harmless no-ops.

Codex `--ephemeral` requests a temporary native session that is deleted on close; it is not a no-op. This does not establish never-persisted or crash-safe ephemeral behavior.
