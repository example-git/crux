...
The full client environment is not forwarded; required resolved values and declared credential inputs can be part of the private runtime. Forwarded state stays in workspace memory, outside public discovery and private-request traffic bodies, and is not saved into server provider/configuration/account stores. The server necessarily sees credentials while executing requests. Explicit client updates publish complete acknowledged revisions, while in-flight work keeps its captured authority. The UI displays the accepted provider source and revision.
