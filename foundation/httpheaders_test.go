...
func TestDefaultUserAgent(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "basic version", version: "0.11.0", want: "Crux-Foundation/0.11.0"},
		{name: "another version", version: "1.0.0", want: "Crux-Foundation/1.0.0"},
	}
...
func TestResolveHeaders_PreservesOtherHeaders(t *testing.T) {
	...
	got := ResolveHeaders(headers, "", "Custom-Client/1.0.0")
	assert.Equal(t, "custom-value", got["X-Custom"])
	assert.Equal(t, "Bearer token", got["Authorization"])
	assert.Equal(t, "Custom-Client/1.0.0", got["User-Agent"])
}
