...
func TestRendererWideCellReanchor(t *testing.T) {
	...
	for _, grapheme := range []bool{false, true} {
		out := render(grapheme)
		for _, column := range []string{"\x1b[3G", "\x1b[5G"} {
			if strings.Count(out, column) != 1 {
				t.Errorf("grapheme=%v: missing per-cell width correction %q in %q", grapheme, column, out)
			}
		}
		if !strings.Contains(out, "\x1b[?7l") || !strings.HasSuffix(out, "\x1b[?7h") {
			t.Errorf("grapheme=%v: wide row must prevent wrapping and restore autowrap: %q", grapheme, out)
		}
	}
}
